...
int refda_adm_ietf_dtnma_agent_init(refda_agent_t *agent)
{
    ...
    if (adm)
    {
        ...
        { // For ./TYPEDEF/column-id
            ...
            obj = refda_register_typedef(adm, cace_amm_idseg_ref_withenum("column-id", REFDA_ADM_IETF_DTNMA_AGENT_ENUM_OBJID_TYPEDEF_COLUMN_ID), objdata);
        ...
        { // For ./VAR/test-var
            ...
            // stored value type:
            {
                ...
                {
                    // Constraint: StringLength(ranges=[-inf,5] | [10,inf])
                    cnst = cace_amm_semtype_cnst_array_push_new(((cace_amm_semtype_use_t *)(&(objdata->val_type))->as_semtype)->constraints);
                    ...
                    cace_amm_range_size_t *range = cace_amm_semtype_cnst_set_strlen(cnst);
                    {
                        ...
                        cace_amm_range_intvl_size_set_max(&intvl, 5);
                        cace_amm_range_intvl_size_seq_push(range->intvls, intvl);
                    }
                    {
                        cace_amm_range_intvl_size_t intvl;
                        cace_amm_range_intvl_size_set_infinite(&intvl);
                        cace_amm_range_intvl_size_set_min(&intvl, 10);
                        cace_amm_range_intvl_size_seq_push(range->intvls, intvl);
                    }
                }
            }
            // initial value:
            cace_ari_set_tstr(&(objdata->value), "testm", true);

            obj = refda_register_var(adm, cace_amm_idseg_ref_withenum("test-var", REFDA_ADM_IETF_DTNMA_AGENT_ENUM_OBJID_VAR_TEST_VAR), objdata);
            // no parameters
        }
